...
class ArcExample extends InteractiveExample {
  ...
  void paintRough(
    ...
    Drawable figure = generator.arc(size.width / 2, size.height / 2, s * 0.8,
        s * 0.8, pi * 0.2, pi * 1.8, true);
...
class CircleExample extends InteractiveExample {
  ...
  void paintRough(
    ...
    double s = min(size.width, size.height);
    Drawable figure =
        generator.circle(size.width / 2, size.height / 2, s * 0.8);
    canvas.drawRough(figure, pathPaint, fillPaint);
  }
}
...
class CurveExample extends InteractiveExample {
  final Paint pathPaint = Paint()
  ...
  void paintRough(
    ...
    for (double i = 0; i <= pi * 6; i += pi / drawConfig.curveStepCount) {
      points.add(PointD(
        size.width / 2 +
            sin(i) * 7 * i +
            drawConfig.offsetSymmetric(drawConfig.maxRandomnessOffset),
        size.height / 2 +
            cos(i) * 7 * i +
            drawConfig.offsetSymmetric(drawConfig.maxRandomnessOffset),
      ));
    ...
    canvas.drawRough(
        Generator(drawConfig, filler).curvePath(points), pathPaint, fillPaint);
  }
}
